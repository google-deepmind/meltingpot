...
_TEST_SUBSTRATE = 'running_with_scissors_in_the_matrix'


def _get_lua_randomization_map():
  """Replaces first row of walls with items randomized by Lua."""
  config = substrate.get_config(_TEST_SUBSTRATE)
...
class GeneralTestCase(parameterized.TestCase):
  ...
  def test_seed_causes_determinism(self, seed):
    ...
    env1 = self.enter_context(substrate.build(config))
    env2 = self.enter_context(substrate.build(config))
